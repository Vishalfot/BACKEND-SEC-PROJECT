...
/**
 * scripts/importPlaces.js — for delhi_v5.xlsx
 * Run this FIRST, then run importClusters.js.
 */
import mongoose from 'mongoose';
import xlsx     from 'xlsx';
import path     from 'path';
...
const parseBool = (v) =>
  v === true || v === 1 || ['true','yes'].includes(String(v).toLowerCase());
...
const importPlaces = async () => {
  try {
    ...
    const sheet = wb.Sheets['Places'];
    if (!sheet) throw new Error('"Places" sheet not found');

    const rows      = xlsx.utils.sheet_to_json(sheet);
    // const validRows = rows.filter(r => r._id && String(r._id).trim());
    const validRows = rows.filter(r => {
  if (!r._id || !String(r._id).trim()) return false;
  if (String(r._id).startsWith('🟡') || String(r._id).startsWith('🟢')) return false;
  const lat = parseFloat(r.latitude);
  const lng = parseFloat(r.longitude);
  return !isNaN(lat) && !isNaN(lng);
});
    console.log(`Rows to import: ${validRows.length}`);

    const ops = validRows.map(row => ({
      updateOne: {
        filter: { _id: String(row._id).trim() },
        update: {
          $set: {
            _id:      String(row._id).trim(),
            name:     row.name,
            ...
            // v5: corrected metro stations (Qutab Minar, Lal Quila, Jorbagh, etc.)
            metro: {
              nearest_station: row.nearest_metro  || '',
              line:            row.metro_line     || '',
              distance_m:      Number(row.metro_distance_m) || 0,
            ...
            // cluster.id filled later by importClusters.js
            cluster: { id: null, anchor_id: null, visit_order: null },
    ...
    console.log(`✅ ${validRows.length} places imported`);
    process.exit(0);
  } catch (err) {
